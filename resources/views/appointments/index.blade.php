...
@php
    ...
    if ($user->role === 'doctor') {
        $doctorFee = optional($filtered->first()?->doctor)->consultation_fee ?? 0;
        $estimatedEarnings = $approvedCount * $doctorFee;
    }

    $statsGrid = $user->role === 'doctor'
        ? 'md:grid-cols-5'
        : 'md:grid-cols-4';
@endphp
    ...
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <input type="text"
                   ...
                   class="rounded-2xl bg-white/10 border-white/10 text-white placeholder:text-slate-400 focus:border-emerald-400 focus:ring-emerald-400">

            <select name="status"
                    class="rounded-2xl bg-slate-800 border-white/10 text-white focus:border-emerald-400 focus:ring-emerald-400">
            ...
            <button class="rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-3 transition">
                Filter
            </button>

        </form>

        <div class="grid grid-cols-1 {{ $statsGrid }} gap-5">
            ...
            <div class="p-6 rounded-3xl bg-white/10 border border-white/10">
                ...
                <h2 class="text-3xl font-black text-cyan-300 mt-2">
            ...
            <div class="p-6 rounded-3xl bg-white/10 border border-white/10">
                ...
                <h2 class="text-3xl font-black text-yellow-300 mt-2">
                    {{ $filtered->where('status', 'Pending')->count() }}
                </h2>
            </div>

            <div class="p-6 rounded-3xl bg-white/10 border border-white/10">
                <p class="text-slate-400 text-sm font-bold">Approved</p>
                <h2 class="text-3xl font-black text-emerald-300 mt-2">
                    {{ $approvedCount }}
                </h2>
            </div>
            ...
            @if($user->role === 'doctor')
                <div class="p-6 rounded-3xl bg-white/10 border border-white/10">
                    ...
                    <h2 class="text-2xl font-black text-white mt-2">
                        ৳ {{ number_format($estimatedEarnings) }}
                    </h2>
                </div>
            @endif
            ...
            <div class="p-6 rounded-3xl bg-white/10 border border-white/10">
                ...
                <div class="space-y-3">
                    @forelse($todayAppointments as $appointment)
                        <div class="p-4 rounded-2xl bg-white/10 text-white flex justify-between items-center">
                            <div>
                                <h3 class="font-black">
                                ...
                                @if($appointment->status === 'Pending')
                                    <div class="grid grid-cols-2 gap-3">
                                        ...
                                        <form method="POST" action="{{ route('appointments.approve', $appointment->id) }}">
                                            ...
                                            <button class="w-full px-4 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold transition">
                                                Approve
                                            </button>
                                        </form>
                                        ...
                                        <form method="POST" action="{{ route('appointments.reject', $appointment->id) }}">
                                            ...
                                            <button class="w-full px-4 py-3 rounded-2xl bg-red-600 hover:bg-red-700 text-white font-bold transition">
                                                Reject
                                            </button>
                                        </form>
            ...
            @empty
                ...
                <div class="p-16 rounded-3xl bg-white/10 text-center">
                    ...
                    @elseif($user->role === 'doctor')
                        <p class="text-slate-300 mt-2">
                            No patient appointments are available yet.
                        </p>
                    @else
                        <p class="text-slate-300 mt-2">
